...
# Haar Cascade file inside project
...
class FaceEngine:
    def __init__(self, threshold=60):
        ...
        # Make deployment errors easier to understand
        if self.detector.empty():
            raise RuntimeError(
                f"Could not load Haar Cascade file: {_CASCADE_PATH}. "
                "Make sure haarcascade_frontalface_default.xml exists in the models folder."
            )
    ...
    def sample_count(self, db, student_id):
        return db.face_samples.count_documents({
            "student_id": student_id
        })

    def delete_student_samples(self, db, student_id):
        db.face_samples.delete_many({
            "student_id": student_id
        })
    ...
    def train(self, db):
        ...
        if not faces:
            ...
            return {
                ...
                "message": (
                    ...
                    "Capture samples for at least one student first."
                )
            }
        ...
        return {
            "success": True,
            "message": (
                f"Model trained on {len(label_map)} student(s), "
                f"{len(faces)} image(s)."
            )
        }

    def load_model(self, db):
        """Load the trained model from MongoDB into memory."""

        doc = db.face_model.find_one({
            "_id": "current"
        })
        ...
        with self._lock:
            for (x, y, w, h) in detections:
                ...
                results.append({
                    ...
                    "confidence": (
                        round(
                            100 - (distance / self.threshold) * 100,
                            1
                        )
                        if known
                        else 0
